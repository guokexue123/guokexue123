    ...
    async with async_playwright() as pw:
        ...
        def on_request(req):
            ...
            if any(host in u for host in DOUYIN_VIDEO_HOSTS) \
                ...
                video_urls_from_requests.append(u)
                print(f"  ✓ [请求拦截] {u[:100]}")
        ...
        async def on_response(response):
            u = response.url
            try:
                if any(path in u for path in DOUYIN_API_PATHS):
                    ct = response.headers.get("content-type", "")
                    if "json" in ct:
                        data = await response.json()
                        urls = extract_urls_from_aweme_json(data)
                        if urls:
                            print(f"  ✓ [API响应] 从 {u[:70]} 提取到 {len(urls)} 个视频URL")
        ...
        # 其次使用请求拦截到的 CDN URL
        if video_urls_from_requests:
            print(f"\n  ✓ 请求拦截到 {len(video_urls_from_requests)} 个视频URL")
            await browser.close()
            return video_urls_from_requests[0], session_cookie_header

        # 策略3: 在页面内直接 fetch 抖音 API（兜底）
        aweme_id = extract_aweme_id(TARGET_URL)
        if aweme_id:
            print(f"  ▶ 兜底：页面内 fetch API，aweme_id={aweme_id}")
            api_url = (
                f"https://www.douyin.com/aweme/v1/feed/"
                f"?aweme_id={aweme_id}&version_code=170400&app_name=douyin_web"
            )
            try:
